...
-- One-shot script. Safe for both fresh projects and re-runs.
--
...
--
-- FIX: Every DROP TRIGGER is now inside a DO block so errors are caught
--   and printed as NOTICE rather than aborting the script.
-- ============================================================================
...
-- Triggers: ALL wrapped in DO blocks because DROP TRIGGER fails if the
-- table does not exist yet, even with IF EXISTS.

DO $$
BEGIN
  DROP TRIGGER IF EXISTS profiles_updated_at ON public.profiles;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'profiles_updated_at trigger: %', SQLERRM;
END;
$$;
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'on_auth_user_created trigger: %', SQLERRM;
END;
$$;

-- Tables (CASCADE handles FK order automatically)
...
CREATE TABLE public.profiles (
  ...
  learning_interest     TEXT        DEFAULT NULL,
...
BEGIN
  INSERT INTO public.rate_limits (
    user_id, date, requests_made, tokens_used, books_started
  )
  VALUES (
    p_user_id, p_date, 1, p_tokens,
...
BEGIN
  ...
  INSERT INTO public.book_history (
    ...
    v_user_id, p_book_id, p_title, p_goal,
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'auth trigger note: %', SQLERRM;
END;
$$;
...
CREATE POLICY "Users read own book history"
  ON public.book_history FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users insert own book history"
  ON public.book_history FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users read own activity"
  ON public.app_activity FOR SELECT USING (auth.uid() = user_id);
...
SELECT
  ...
  (SELECT generation_mode FROM public.book_history
...
INSERT INTO public.profiles (
  id, email, full_name, profession, learning_interest,
  plan, plan_expires_at, books_created, total_words_generated
)
SELECT
  u.id,
  COALESCE(u.email, ''),
  ...
  u.raw_user_meta_data->>'learning_interest',
  'yearly',
  NOW() + INTERVAL '1 year',
  0,
  0
...
-- Expected: 6 rows. If fewer, scroll up for any NOTICE messages.
-- ============================================================================
